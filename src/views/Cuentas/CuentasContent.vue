  ...
  <div class="w-full max-w-5xl mx-auto bg-[#e7eef6] rounded-2xl p-4 md:p-6 mt-8">
    ...
    <div v-else class="flex flex-col md:flex-row gap-6 md:gap-10 items-start justify-center w-full">
      ...
      <aside class="flex flex-col gap-3 min-w-[220px] max-w-[270px] w-full md:w-[250px]">
        ...
        >
          <span class="w-9 h-9 rounded-full flex items-center justify-center bg-[#01a7e4] text-white text-xl font-bold transition-all duration-300 group-hover:bg-[#0a2540]">
            <svg v-if="cuenta.tipoCuentaNombre?.toLowerCase().includes('ahorro')" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          ...
          >
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-3 mb-8">
              ...
              <div>
                ...
                <div class="text-gray-600 text-sm mb-2">Q {{ cuentaSeleccionada.saldoCuenta?.toFixed(2) }}</div>
                <div class="text-[#15385c] text-sm font-semibold">Tipo última transfe</div>
                <div class="text-gray-600 text-sm">--</div>
              </div>
            </div>
            <div class="flex gap-8 w-full justify-center mt-2">
              <button class="bg-[#01a7e4] hover:bg-[#008fbe] active:scale-95 text-white font-bold py-2 px-8 rounded-lg shadow transition-all duration-200">
                Transferencia
              </button>
              <button class="bg-[#f754a5] hover:bg-[#db2883] active:scale-95 text-white font-bold py-2 px-8 rounded-lg shadow transition-all duration-200">
                Historial
              </button>
            </div>
          </div>
...
async function fetchCuentas() {
  try {
    ...
    error.value = 'No se pudieron cargar tus cuentas'
    cuentas.value = []
  } finally {
    loading.value = false
  }
